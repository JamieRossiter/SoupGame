...
Galv.MSE.makeSound = function(txt) {
	...
	var obj = {
		name: arr[0],
		pan: Number(arr[3]) || 0,
		pitch: Math.floor(Math.random() * (120 - 70 + 1)) + 70,
		volume: Number(arr[1])
	};
	return obj;
};
...
Window_Message.prototype.processNormalCharacter = function(textState) {
	if (!this._showFast) {
		this.startWait(5); // Added by Jamie Rossiter - causes text speed to slow down
		if (this.delayTime >= Galv.MSE.delay) {
			// Added by Jamie Rossiter - different sounds play based on the character type
			if(textState.text[textState.index] == " "){
				$gameMessage.msgSe.name = "typewriter_space";
				$gameMessage.msgSe.pitch = Math.floor(Math.random() * (100 - 95 + 1)) + 95;
			} else {
				$gameMessage.msgSe.name = "typewriter";
				$gameMessage.msgSe.pitch = Math.floor(Math.random() * (110 - 90 + 1)) + 90;
			}
			AudioManager.playSe($gameMessage.msgSe);
			this.delayTime = 0;	
		};
	} else {
		// Added by Jamie Rossiter - bell sound plays if the player skips dialogue
		$gameMessage.msgSe.name = "typewriter_bell";
		$gameMessage.msgSe.volume = 50;
		$gameMessage.msgSe.pitch = Math.floor(Math.random() * (100 - 95 + 1)) + 95;
		AudioManager.playSe($gameMessage.msgSe);
	}
