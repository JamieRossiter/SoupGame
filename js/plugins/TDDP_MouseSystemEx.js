...
(function($) {
    ...
    */
    ...
    /**
    * Perform check for event under mouse and perform functions depending on parsed notetag properties. Edited by Jamie Rossiter - function now checks whether sprite is under mouse as oppose to event.
    ...
    */
    TouchInput._checkForEventUnderMouse = function(pageX, pageY) {
        ...
        if ($.conditionsValidForMouseHoverCheck()) {
            ...
            /* Filter sprites that have mouse hovered over them */
            var sprites = SceneManager._scene._spriteset._characterSprites.filter(sprite => {
                if(sprite._character && "TDDP_MS" in sprite._character){
                    let validX = mouseX >= (sprite.x - sprite.width / 2) && mouseX <= (sprite.x - sprite.width / 2) + sprite.width;
                    let validY = mouseY >= (sprite.y - sprite.height) && mouseY <= (sprite.y - sprite.height) + sprite.height;
                    // if(validX && validY){
            ...
            var events = sprites.map(ev => ev._character);
            events.reverse().forEach(function(game_event) {
                ...
                /* If the mouse button is clicked while the mouse is hovered over the sprite, run the event */
                if(TouchInput.isPressed() && !$gameMessage.isBusy()){
                    game_event.start();
                }
    ...
    */
    ...
    // Added by Jamie Rossiter. This replaces the on trigger function.
