...
Game_Character.prototype.moveToEvent = function(eventId, collision) {
  ...
  if (eventId === 0) {
    ...
    var ev = $gameMap.event(eventId);
    if (!ev) return;
    if($gamePlayer.isNearlyCollidedWithCharacters($gamePlayer.x, $gamePlayer.y)) return; // Added by Jamie Rossiter - checks to see if the player is 1 tile away from the character.
    var x = ev.x;
    var y = ev.y;
    this.moveToPoint(x, y, collision);
  }
